...
export default function TrendRadar() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("watchlist", false);
        if (res && res.value) setWatchlist(new Set(JSON.parse(res.value)));
  ...
  const toggleWatch = (id) => {
    setWatchlist((prev) => {
      ...
      window.storage.set("watchlist", JSON.stringify(Array.from(next)), false).catch(() => {});
      return next;
    });
  };
